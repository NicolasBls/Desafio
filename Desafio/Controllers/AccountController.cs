...
namespace Desafio.Controllers
{
    ...
    public class AccountController : Controller
    {
       ...
       //[ValidateAntiForgeryToken]
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl, string cpf)
        {
            var pessoa = db.Pessoa.FirstOrDefault(x=>x.email == model.Email && x.cpf == cpf);
            if (pessoa != null)
            {

                FormsAuthentication.SetAuthCookie(pessoa.nome.ToString(), false);
                //FormsAuthentication.RedirectFromLoginPage(pessoa.nome, true);
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl, string nome, string cpf, DateTime data)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser { UserName = nome, Email = model.Email };
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                        var pessoa = new Pessoa {nome = nome, cpf = cpf, email = model.Email, dataNascimento = data };
